...
class FakeAccountVaultServer {
  ...
  Future<void> _handleLogin(HttpRequest request) async {
    ...
    final identifier = (payload['identifier'] ?? payload['email'] ?? '')
        .toString()
        .trim();
  ...
  Future<void> _handleVerifyMfa(HttpRequest request) async {
    final payload = await _decodeJson(request);
    final ticket = (payload['mfaToken'] ?? payload['mfa_ticket'] ?? '')
        .toString()
        .trim();
    final code = (payload['code'] ?? payload['totpCode'] ?? '')
        .toString()
        .trim();
  ...
  Future<void> _handleSession(HttpRequest request) async {
    ...
    await _writeJson(request.response, HttpStatus.ok, <String, Object?>{
      'user': _userPayload(mfaEnabled: requireMfa),
    });
  }
  ...
  Future<void> _handleModels(HttpRequest request) async {
    lastAiGatewayAuthorization = request.headers.value(
  ...
  Future<void> _handleVault(HttpRequest request) async {
    ...
    final data = switch (path) {
      ...
      'ollama' => <String, Object?>{'OLLAMA_API_KEY': ollamaCloudApiKey},
      _ => <String, Object?>{'UNMAPPED_KEY': 'ignored-value'},
    };
    await _writeJson(request.response, HttpStatus.ok, <String, Object?>{
